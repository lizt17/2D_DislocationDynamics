...
// Parameters
const double tau_nuc = 1000e6 / mu_SI;
...
int main() {
    ...
    for (int kInc = 0; kInc < Nsteps; ++kInc) 
    {
        ...
        if (rss_source >= tau_nuc) 
        {
            ...
            std::cout << "Nucleating dislocation, rss_source = " << rss_source << std::endl;
            Dislocation nucleatedDis = {nextDisID, r_source, 0.0};
            nextDisID++;
            disArr.push_back(nucleatedDis);
            ++Nd;
        }
        double dt = 10.0;
        if (Nd > 0) 
        {
            ...
            dt = Vmax>1e-3? dxMax / Vmax : dxMax/1e-3;
        ...
        if (kInc % outputInterval == 0) 
        {
            ...
            << ", " << (Kapp-KD) * unitSIF / 1e6 << ", " << back_stress <<"\n";
            std::ofstream fileDis(outputDir + "dislocation_" + std::to_string(outFileIndex)+".csv", std::ios::app);
            for (int i = 0; i < Nd; ++i)
            {
